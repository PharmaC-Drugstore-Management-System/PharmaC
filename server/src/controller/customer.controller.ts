...
const controller = {
  ...
  updateCustomer: async (req: any, res: any) => {
    try {
      ...
      const updatedCustomer = await customer_service.update(
        customerId,
        req.body
      );
  ...
  addCustomerPoint: async (req: any, res: any) => {
    try {
      const { id } = req.params;
      ...
      const pointsToAdd = parseInt(point);
      ...
      console.log("Adding points:", pointsToAdd, "to customer:", customerId);
      const response = await customer_service.addPointById(customerId, pointsToAdd);
      
      return res.status(200).json({ 
        status: true, 
        data: response,
        message: `Successfully added ${pointsToAdd} points to customer ${customerId}`
      });
    } catch (error) {
