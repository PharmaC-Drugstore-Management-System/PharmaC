...
class Forecaster:
    def __init__(self, forecast_days: int = config.FORECAST_DAYS):
        ...
        self.data = None  # Will store data from backend integration
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, List[str]]:
        """Load and prepare data from backend or CSV files."""
        try:
            # Check if data was loaded from backend (via main.py database integration)
            if hasattr(self, 'data') and self.data is not None and not self.data.empty:
                self.logger.info("Using data loaded from backend database")
                combined_data = self.data.copy()
                
                # Get drug names (all columns except 'date')
                drug_names = [col for col in combined_data.columns if col != 'date']
                
                # Basic validation and cleaning
                combined_data['date'] = pd.to_datetime(combined_data['date'])
                combined_data = combined_data.sort_values('date').reset_index(drop=True)
                
                # Fill NaN values with 0 for drug columns
                for drug in drug_names:
                    combined_data[drug] = pd.to_numeric(combined_data[drug], errors='coerce').fillna(0)
                    # Ensure non-negative values
                    combined_data[drug] = combined_data[drug].clip(lower=0)
                
                self.logger.info(f"Backend data loaded: {len(combined_data)} records, {len(drug_names)} products")
                self.logger.info(f"Date range: {combined_data['date'].min()} to {combined_data['date'].max()}")
                self.logger.info(f"Products: {drug_names[:10]}{'...' if len(drug_names) > 10 else ''}")
                
                return combined_data, drug_names
            
            # Fallback to CSV files or API
            else:
                self.logger.info("Loading data from CSV files or API...")
                
                # Try API first if configured
                if hasattr(config, 'API_ENDPOINT') and config.API_ENDPOINT:
                    try:
                        import requests
                        self.logger.info(f"Fetching sales data from API: {config.API_ENDPOINT}")
                        response = requests.get(config.API_ENDPOINT, timeout=30)
                        ...
                        drug_names = [col for col in df.columns if col != 'date']
                        return df, drug_names
                        
                    except Exception as e:
                        self.logger.warning(f"API data fetch failed: {e}, falling back to CSV files")
                
                # Fallback to CSV files
                combined_data = self.data_processor.combine_monthly_files()
                combined_data = self.data_processor.validate_data_quality(combined_data)
                drug_names = [col for col in combined_data.columns 
                            if col not in ['date', 'source_file', 'Year', 'Month', 'Hour', 'Weekday Name']]
                
                self.logger.info(f"CSV data loaded: {len(combined_data)} records, {len(drug_names)} products")
                return combined_data, drug_names
    ...
    def train_evaluate_models(self, drug_data: pd.DataFrame, drug_name: str, model: str = None) -> Dict[str, Dict]:
        ...
        try:
            ...
            # If specific model is requested, only run that model
            models_to_run = []
            if model:
                if model.upper() == 'ARIMA':
                    models_to_run = ['ARIMA']
                elif model.upper() == 'PROPHET':
                    models_to_run = ['Prophet']
                elif model.upper() == 'CATBOOST':
                    models_to_run = ['CatBoost']
                else:
                    self.logger.warning(f"Unknown model '{model}', running all models")
                    models_to_run = ['ARIMA', 'Prophet', 'CatBoost']
            else:
                models_to_run = ['ARIMA', 'Prophet', 'CatBoost']
    ...
    def generate_predictions(self, drug_data: pd.DataFrame, drug_name: str, model: str = None) -> Dict[str, Dict]:
    ...
    def run_complete_analysis(self, drug_filter: List[str] = None, model: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for i, drug_name in enumerate(drug_names, 1):
                ...
                try:
                    ...
                    evaluation_results = self.train_evaluate_models(drug_data, drug_name, model)
                    
                    # Generate final predictions
                    predictions = self.generate_predictions(drug_data, drug_name, model)
